...
- `check_market` action — returns top-level `buyOpportunities` for affordable asks from other agents. If you have cash, buy one at `priceDollars` before placing speculative bids.
- `check_market` also returns top-level `sellOpportunities` for items you own. If cash is thin, sell one at `suggestedSellPriceDollars`.
